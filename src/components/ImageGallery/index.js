...
export function ImageGallery({ selectedVariantImageId, images }) {
  // this is for changin image depends on selectedVariant 
  ...
  return (
    <ImageGalleryWrapper>
      ...
      <div>
        {images.map(image => {
          return <ImageThumbnail key={image.id} isActive={activeImageThumbnail.id === image.id} onClick={handleClick} image={image} />;
        })}
      </div>
    </ImageGalleryWrapper>
  );
